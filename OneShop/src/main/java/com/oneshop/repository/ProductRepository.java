...
public interface ProductRepository extends JpaRepository<Product, Long>, JpaSpecificationExecutor<Product> {
	...
	@Query("SELECT p FROM Product p WHERE p.shop.id = :shopId "
			...
			"AND (:categoryId IS NULL OR p.category.id = :categoryId) " + // ✅ Xử lý Danh mục
			"AND (:brand IS NULL OR p.brand.name LIKE %:brand%)") // ✅ Xử lý Thương hiệu
	 ...
	 */
	@Query("SELECT DISTINCT p.brand.name FROM Product p WHERE p.brand IS NOT NULL AND p.brand.name <> ''")
	...
	/**
	 * Tìm kiếm và lọc sản phẩm public (nên dùng Specification thay thế). Sửa kiểu
	 * giá thành BigDecimal.
	 */
	@Query("SELECT p FROM Product p WHERE p.published = true "
			+ "AND (?1 IS NULL OR LOWER(p.name) LIKE LOWER(CONCAT('%', ?1, '%'))) "
			+ "AND (?2 IS NULL OR p.category.id = ?2) " + "AND (?3 IS NULL OR p.brand.brandId = ?3) "
			+ "AND (?4 IS NULL OR p.price >= ?4) " + // BigDecimal
			"AND (?5 IS NULL OR p.price <= ?5)") // BigDecimal
	List<Product> searchAndFilterPublic(String name, Long categoryId, Long brandId, BigDecimal minPrice,
			BigDecimal maxPrice); // Sửa kiểu giá
}